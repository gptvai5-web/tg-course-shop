...
serve(async (req) => {
  ...
  try {
    ...
    const { course_id, combo_id, cycle_ids, amount, coupon_code, cust_name, cust_email, cust_phone } = await req.json();
    ...
    const { error: insertError } = await adminClient.from("payments").insert({
      ...
      cycle_ids: cycle_ids && cycle_ids.length > 0 ? cycle_ids : null,
